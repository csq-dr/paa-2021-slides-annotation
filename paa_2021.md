...
One benefit of using the NL distribution is that, this distribution allows asymmetric CI for the posterior estimates, and 95% CI of point estimates can cover the potential biased prevalence within the uncertainty. 

The graph on the right shows relationship between observed $y$ and estimates of $\theta$ for NL model vs the true data model.  Here we assume sampling error $s = 0.01$ and $se = 0.9, sp = 1$ for both models. Comparing to the conflict between logit-normal model and true data model, NL model always includes the estimates based on the true data model, while the point estimates from NL model align to the line of $\hat\theta = y$ as per aims.

# Slide 9: Simulation

Before we apply this NL model to FPEM, we conduct a simulation to study the effect of misclassification error on estimation with some generated data. In the simulation, we fixed the true $\theta$ to be 0.3, 0.5, 0.7, and set various groups of true misclassification and assumed misclassification, corresponding to situations that (1) there is no misclassification at all, (2) we make a wrong guess of the misclassification regardless of misclassification existing or not; (3) we happen to make a correct guess in the presence of misclassification. 

We set the true data generation process as a binomial with sample size $n$, and use it as the true model in the simulation. Posterior estimates and 95% credible intervals are obtained from the true model and the two assumptions.

This simulation results in findings as expected: first, point estimates are comparable between the logit-normal and NL model regardless of the bias. __Second, 95% credible intervals from NL model is conservative (?)__. Lastly, the NL model improves upon logit-normal model in terms of coverage of 95% credible interval when misclassification is present and accounted for.

# Slide 10: FPEM implementation

The simulation results is as expected so we implement the NL model in FPEM. The NL model is added to FPEM for global runs and only applied to DHS data points due to evidence of misclassification are for DHS data collection. Two groups of misclassification assumptions informed by Ghana and Nepal follow-up study are produced in the implementation.

As expected, we find that: the estimated trends and bounds vary based on the misclassification assumption. For example, in these two graphs of Nepal, the observed DHS modern use are around 40% and lead to a decrease in the forecast with the assumption $se^a = sp^a = 0.9$. However, the effect of changes from logit-normal to NL model is variable, for example, we don't see much changes in Nepal with the assumption $se^a = 0.9, sp^a = 1$. This is due to estimates being not only informed by DHS data but also informed by the process model and additional non-DHS data that still has the logit-normal model.

# Slide 11: Summary

This is the case study of how to estimate a proportion for a population at a given time with unknown misclassification. Due to the limited misclassification evidence, we propose a new model to account for the potential misclassification error with plausible uncertainty associated with estimates from the model. Our simulation study indicates some improvement with misclassification being presented. The implementation of the new model in FPEM shows changes of estimated trends and forecast, however the effect of changes is variable due to the complexity of the model. 