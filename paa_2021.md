...
We find that: difference in FPEM fit between the two models are more variable and generally smaller comparing to the simulations. That's what we see in the results for Nepal on the right, where the bounds based on the two different data models are approximately equal. It's possibly because FPEM-based estimates being informed not only by DHS data but also by process model and additional non-DHS data sources.

# Slide 11: FPEM implementation (2)
